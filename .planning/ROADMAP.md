...
- [ ] **Phase 1: Foundation & Setup** - Analog.js scaffold, @foblex/flow, Tailwind, 3-panel layout, types, mock backend
- [ ] **Phase 2: Flow Editor** - Custom nodes, drag & drop, edges, mini-map, zoom, FlowService
- [ ] **Phase 3: Chat & SSE Integration** - Chat UI, streaming, typing indicator, node highlighting, ChatService
- [ ] **Phase 4: Configuration & Polish** - Config panel, save/load flow, reset/new chat buttons

## Phase Details

### Phase 1: Foundation & Setup
**Goal**: A running Analog.js app with the correct flow library installed, three-panel layout visible, TypeScript types defined, and a mock backend that unblocks all parallel development
...
**Success Criteria** (what must be TRUE):
  ...
  4. `models/types.ts` exports FlowNode, FlowEdge, ChatMessage, and AgentConfig interfaces
  5. A mock Nitro API route returns a scripted SSE sequence (including a `node_active` event) when called
**Plans**: TBD
...
**Goal**: A working chat playground where messages stream in token-by-token via SSE, and the corresponding flow nodes illuminate in real time as the agent executes
**Depends on**: Phase 2
...
**Success Criteria** (what must be TRUE):
  ...
  4. "Nueva Conversacion" clears the chat history and starts a fresh session
**Plans**: TBD
...
| 2. Flow Editor | 0/? | Not started | - |
| 3. Chat & SSE Integration | 0/? | Not started | - |
