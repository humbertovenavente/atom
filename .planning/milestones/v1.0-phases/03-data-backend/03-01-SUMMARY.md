...
plan: 01
subsystem: database
tags: [mongoose, mongodb, google-genai, embeddings, vector-search, seed]
...
provides:
  - Mongoose Vehicle model with 768-dim embedding field (gemini-embedding-001)
...
# Tech tracking
tech-stack:
  added: ["@google/genai@^1.43.0", tsx@4.21.0]
  patterns:
    - mongoose.models['Name'] || mongoose.model('Name', schema) registration pattern for hot-reload safety
    - embedBatch() with 20-text chunks and 1-second inter-batch delay for Gemini rate limiting
...
key-decisions:
  - "gemini-embedding-001 chosen (768 dims, cosine similarity) — uses existing LLM_API_KEY, sufficient quality for demo dataset of 150 docs"
  ...
  - "LLM_API_KEY used for @google/genai client"
...
patterns-established:
  ...
  - "Gemini embeddings use LLM_API_KEY env var via @google/genai"
...
**Three Mongoose models (Vehicle/FAQ/DateSlot) with Gemini gemini-embedding-001 embeddings, npm run seed command that loads 100 vehicles + 50 FAQs + 31 date slots and creates Atlas Vector Search indexes**
...
- Vehicle Mongoose model with 16 camelCase fields mapped from Spanish JSON names plus 768-dim embedding array
...
- Comprehensive seed script that generates embeddings via Gemini (@google/genai), does drop-and-reseed, and creates+polls Atlas Vector Search indexes until READY
...
- **Embedding model:** gemini-embedding-001 (768 dims) via @google/genai — uses existing LLM_API_KEY, sufficient for 150-document hackathon dataset. Cosine similarity metric.
...
- **LLM_API_KEY** must be set in `.env` — this is your Gemini API key
