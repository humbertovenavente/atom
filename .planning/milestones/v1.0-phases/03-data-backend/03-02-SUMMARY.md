...
plan: 02
subsystem: api
tags: [mongodb, mongoose, nitro, h3, google-genai, vector-search, sessions]
...
tech-stack:
  ...
  patterns:
    ...
    - "Lazy Gemini client: getGenAI() factory via @google/genai, not module-level instantiation"
...
key-decisions:
  ...
  - "Lazy Gemini client using LLM_API_KEY via @google/genai per existing pattern"
...
**Four MongoDB-backed API routes (vehicles, dates, sessions CRUD) plus VectorSearchService with $vectorSearch aggregation using gemini-embedding-001 for Phase 4 chat specialists**
...
- All embedding vectors excluded from API responses; lazy Gemini client uses LLM_API_KEY via @google/genai
...
- Lazy Gemini client initialization: getGenAI() factory via @google/genai prevents module-load failures when LLM_API_KEY not set in test/non-chat contexts
